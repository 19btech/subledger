...
5. looks up the batch's active `InstrumentAttribute` rows in one query, keyed by
   (instrumentId, attributeId), and which attributes are reclassable (`Attributes`, cached per tenant
   for 5 minutes). Models get attribute values from their events, per sub-instrument — the worker
   does not add any;
