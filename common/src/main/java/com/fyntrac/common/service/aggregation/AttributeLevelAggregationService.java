...
public class AttributeLevelAggregationService extends CacheBasedService<AttributeLevelLtd> {
     ...
     */
    public AttributeLevelLtd getBalance(String instrumentId, String attributeId, String metric, int accountingPeriodId) {
        String tenantId = dataService.getTenantId();
        String key = getKey(tenantId, accountingPeriodId, instrumentId, attributeId, metric);
        log.info("Fetching balance for Instrument: {}, Attribute: {}, Metric: {}, Period: {}, Tenant: {}",
                instrumentId, attributeId, metric, accountingPeriodId, tenantId);
        ...
        try {
            ltd = dataService.findOne(query, AttributeLevelLtd.class);
            if (ltd != null) {
            ...
            log.error("MongoDB query failed for Instrument: {}, Attribute: {}, Metric: {}, Period: {}",
                    instrumentId, attributeId, metric, accountingPeriodId, e);
     ...
     */
    public List<AttributeLevelLtd> aggregate(TransactionActivity activity) {
        ...
        metrics.forEach(aggregation -> {
            ...
            try {
                ltd = this.getBalance(activity.getInstrumentId(), activity.getAttributeId(), aggregation.getMetricName(), activity.getPeriodId());

                if (ltd == null) {
                    log.warn("No balance found for current period. Fetching from previous period.");
                    ltd = this.getBalance(activity.getInstrumentId(), activity.getAttributeId(), aggregation.getMetricName(), activity.getAccountingPeriod().getPreviousAccountingPeriodId());
            ...
            if (currentPeriodLtdFound) {
                ...
                try {
                    dataService.save(attributeLevelLtd);
                    memcachedRepository.putInCache(key, attributeLevelLtd);
     ...
     * @return A list of {@link AttributeLevelLtd} records after aggregation.
     */
    public List<AttributeLevelLtd> aggregate(Collection<TransactionActivity> activities) {
    ...
    private String getKey(String tenantId, int accountingPeriodId, String instrumentId, String attributeId, String metricName) {
        return String.format("%s-%d-%s-%s-%s", tenantId, accountingPeriodId, instrumentId.toUpperCase(), attributeId.toUpperCase(), metricName.toUpperCase());
    }
}
