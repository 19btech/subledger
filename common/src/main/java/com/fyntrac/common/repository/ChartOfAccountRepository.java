...
public interface ChartOfAccountRepository extends MongoRepository<ChartOfAccount, String> {
    ...
    /**
     * Fetches all active (non soft-deleted) chart of account entries. Records missing the
     * isDeleted field entirely (pre-existing data) are treated as active.
     */
    @Query("{ '$or': [ { 'isDeleted': false }, { 'isDeleted': { '$exists': false } } ] }")
    List<ChartOfAccount> findByIsDeletedFalse();

    /**
     * Soft-deletes a single chart of account document by its unique database ID.
     *
