...
@Service
@Slf4j
public class ExcelModelDiagnosticExecutor {
    ...
    @Autowired
    public ExcelModelDiagnosticExecutor(ExcelFileService excelFileService,
    ...
    public Records.ModelOutputData execute(ModelWorkflowContext context) throws Throwable {
        ...
        loadTransactions(context,upperCaseTransactions, workbook);
        ...
        loadMetrics(context,transactionList, workbook);
    ...
    private List<TransactionActivity> getActivities(ModelWorkflowContext context, List<String> upperCaseTransactions) {
        ...
        String modelId = context.getExcelModel().model().getId();
        for(Records.InstrumentAttributeModelRecord instrumentAttribute : context.getCurrentInstrumentAttribute()) {
            List<TransactionActivity> transactionActivities =  this.transactionService.fetchTransactions(upperCaseTransactions,instrumentAttribute.instrumentId(), instrumentAttribute.attributeId(),  context.getExecutionDate(), modelId);
            activityList.addAll(transactionActivities);
        }
        return  activityList;
    }

    private List<TransactionActivity> getModelActivities(ModelWorkflowContext context, List<String> upperCaseTransactions) {
        List<TransactionActivity> activityList = new ArrayList<>(0);
        String modelId = context.getExcelModel().model().getId();
        for(Records.InstrumentAttributeModelRecord instrumentAttribute : context.getCurrentInstrumentAttribute()) {
            List<TransactionActivity> transactionActivities =  this.transactionService.fetchTransactionsByModelId(instrumentAttribute.instrumentId(), instrumentAttribute.attributeId(),  context.getExecutionDate(), modelId);
            activityList.addAll(transactionActivities);
        }
        return  activityList;
    }
    private void  loadTransactions(ModelWorkflowContext context, List<String> transactionList, Workbook workbook) throws Throwable {

        List<TransactionActivity> transactionActivities =  getActivities(context, transactionList);
        ...
        for (String transactionName : transactionList) {
    ...
    private void  loadMetrics(ModelWorkflowContext context, List<String> transactionList, Workbook workbook) throws Throwable {
        ...
        List<TransactionActivity> modelActivities = this.getModelActivities(context, transactionList);
        Map<String, BigDecimal> metricLevlBalances = new HashMap<>();
        Map<String, BigDecimal> instrumentLevlBalances = new HashMap<>();
        Map<String, BigDecimal> attributeLevlBalances = new HashMap<>();
        for (TransactionActivity activity : modelActivities) {
            BigDecimal amount = activity.getAmount();
            List<Aggregation> metricList = this.aggregationService
                    .getMetrics(activity.getTransactionName().toUpperCase());

            for (Aggregation agg : metricList) {
                String metricKey = agg.getMetricName().toUpperCase();
                String attributeKey = String.format("%s-%s-%s", metricKey, activity.getInstrumentId().toUpperCase(), activity.getAttributeId().toUpperCase());
                String instrumentKey = String.format("%s-%s", metricKey, activity.getInstrumentId().toUpperCase());

                // Use merge to simplify accumulation
                metricLevlBalances.merge(metricKey, amount, BigDecimal::add);
                instrumentLevlBalances.merge(instrumentKey, amount, BigDecimal::add);
                attributeLevlBalances.merge(attributeKey, amount, BigDecimal::add);
            }
        }
        ...
        switch (aggregationLevel){
            case AggregationLevel.ATTRIBUTE -> {
                ...
                for(AttributeLevelLtd ltd : ltds) {
                    String attributeKey = String.format("%s-%s-%s", ltd.getMetricName().toUpperCase(), ltd.getInstrumentId().toUpperCase(), ltd.getAttributeId().toUpperCase());
                    BigDecimal attributeBalance = attributeLevlBalances.get(attributeKey);
                    if(attributeBalance != null) {
                        BigDecimal currentActivity = Objects.requireNonNullElse(ltd.getBalance().getActivity(), BigDecimal.ZERO);
                        BigDecimal beginningBalance = Objects.requireNonNullElse(ltd.getBalance().getBeginningBalance(), BigDecimal.ZERO);

                        // Always subtract absolute value of attributeBalance
                        BigDecimal newActivity;
                        if (currentActivity.abs().compareTo(attributeBalance.abs()) == 0) {
                            newActivity = BigDecimal.ZERO;
                        } else {
                            newActivity = currentActivity.subtract(attributeBalance);
                        }
                        ltd.getBalance().setActivity(newActivity);

                        // endingBalance = beginningBalance + newActivity
                        ltd.getBalance().setEndingBalance(beginningBalance.add(newActivity));
                    }
            ...
            case AggregationLevel.INSTRUMENT -> {
                ...
                for(InstrumentLevelLtd ltd : ltds) {
                    String instrumentKey = String.format("%s-%s", ltd.getMetricName().toUpperCase(), ltd.getInstrumentId().toUpperCase());
                    BigDecimal instrumentBalance = instrumentLevlBalances.get(instrumentKey);
                    if(instrumentBalance != null) {
                        BigDecimal currentActivity = Objects.requireNonNullElse(ltd.getBalance().getActivity(), BigDecimal.ZERO);
                        BigDecimal beginningBalance = Objects.requireNonNullElse(ltd.getBalance().getBeginningBalance(), BigDecimal.ZERO);

                        // Always subtract absolute value of attributeBalance
                        BigDecimal newActivity;
                        if (currentActivity.abs().compareTo(instrumentBalance.abs()) == 0) {
                            newActivity = BigDecimal.ZERO;
                        } else {
                            newActivity = currentActivity.subtract(instrumentBalance);
                        }
                        ltd.getBalance().setActivity(newActivity);
            ...
            case AggregationLevel.TENANT ->  {
                ...
                for(MetricLevelLtd ltd : ltds) {
                    String metricKey = String.format("%s", ltd.getMetricName().toUpperCase());
                    BigDecimal metricBalance = metricLevlBalances.get(metricKey);
                    if(metricBalance != null) {
                        BigDecimal currentActivity = Objects.requireNonNullElse(ltd.getBalance().getActivity(), BigDecimal.ZERO);
                        BigDecimal beginningBalance = Objects.requireNonNullElse(ltd.getBalance().getBeginningBalance(), BigDecimal.ZERO);
                        ...
                        // Always subtract absolute value of attributeBalance
                        BigDecimal newActivity;
                        if (currentActivity.abs().compareTo(metricBalance.abs()) == 0) {
                            newActivity = BigDecimal.ZERO;
                        } else {
                            newActivity = currentActivity.subtract(metricBalance);
                        }
